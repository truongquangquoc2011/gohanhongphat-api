// src/shared/utils/multer.utils.ts
...
export const pdfFileFilter = (req, file, cb) => {
  if (SUPPORTED_PDF_MIME.includes(file.mimetype)) return cb(null, true)
  return cb(new Error('Unsupported MIME type. Allowed: application/pdf'), false)
}
